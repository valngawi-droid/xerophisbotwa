...
async function startConnection(options = {}) {
  ...
  if (usePairingCode && !sock.authState.creds.registered) {
    const { normalizePhone, isValidPhone, countryOf } = await import(
      "./lib/xerophis-region.js"
    ).catch(() => ({
      normalizePhone: (n) => String(n || "").replace(/[^0-9]/g, ""),
      isValidPhone: (n) => String(n || "").replace(/[^0-9]/g, "").length >= 8,
      countryOf: () => "Internasional",
    }));

    let phoneNumber = normalizePhone(pairingNumber || "");

    // Nomor negara MANAPUN diterima: Mali (223...), Rusia (7...), dll.
    for (let attempt = 0; attempt < 3 && !isValidPhone(phoneNumber); attempt++) {
      console.log("");
      if (phoneNumber) {
        colors.logger.warn(
          "pairing",
          `nomor "${phoneNumber}" tidak valid, coba lagi`,
        );
      } else {
        colors.logger.warn("pairing", "nomor pairing belum diatur di config/.env");
      }
      console.log(
        colors.chalk.gray(
      ...
      phoneNumber = normalizePhone(answer);
    }

    if (!isValidPhone(phoneNumber)) {
      colors.logger.error(
        "pairing",
        "nomor tidak valid setelah 3x percobaan — restart dan coba lagi",
      );
      return sock;
    }
    ...
    colors.logger.info(
      "pairing",
      `meminta kode untuk +${phoneNumber} (${countryOf(phoneNumber)})`,
    );

    try {
      await new Promise((resolve) => setTimeout(resolve, 2000));
      const code = await sock.requestPairingCode(phoneNumber, "XEROPHIS");
      console.log("");
      console.log(
        colors.createBanner(
          [
  ...
  sock.ev.on("connection.update", async (u) => {
    ...
    if (c === S.O) {
      ...
      colors.logger.info(
        "bot",
        `${config.bot?.name || "Xerophis Bot"} (${n || "?"}) · WA v${version.join(".")}`,
      );
