...
function createContextInfo(jid, text, title = "", body = "", thumbnail = null) {
  ...
  if (jid && text) {
    ...
    contextInfo.stanzaId = "XEROPHIS" + Date.now();
  }
...
async function serialize(sock, msg, store = {}) {
  ...
  m.botName = sock.user?.name || config.bot?.name || "Xerophis Bot";
  m.messageId = m.id;
  m.chatId = m.chat;
  m.senderId = m.sender;
    ...
    if (replyVariant === 2) {
      ...
      return sock.sendPreview(
        m.chat,
        {
          ...
          title: config.bot?.name || "Xerophis Bot",
          description:
        ...
        },
        {
          ...
          contextInfo: {
            ...
            forwardedNewsletterMessageInfo: {
              ...
              newsletterName: config.saluran?.name || config.bot?.name || "Xerophis Bot",
