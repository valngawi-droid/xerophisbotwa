...
function getRpgContextInfo(title, body) {
  ...
  if (title || body) {
    base.externalAdReply = {
      title: title || config.bot?.name || "Xerophis RPG",
      body: body || "",
...
function createFakeQuoted(botName = "Xerophis Bot", verified = true) {
  return {
    key: {
      fromMe: false,
