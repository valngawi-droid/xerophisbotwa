...
import { normalizePhone } from "./xerophis-region.js";
...
footer: Powered by Xerophis AI (opsional)
[/RICH:CODE]
...
[RICH:LIST]
title: Info Bot
rows: Nama | Xerophis AI;; Versi | 4.0.0;; Developer | Pall
footer: © Xerophis AI (opsional)
[/RICH:LIST]

5. STICKER (saat user minta sticker, atau untuk ekspresi emosi):
[RICH:STICKER]
url: https://iili.io/BPBdFuj.md.jpg
packname: Xerophis AI (opsional)
  ...
  try {
    ...
    if (rich.type === "STICKER") {
      ...
      await sock.sendImageAsSticker(jid, stickerInput, quoted, {
        packname: packname || config.bot?.name || "Xerophis Bot",
...
function detectIntentFromMessage(msg, m) {
  ...
  const phoneMatch = msg.match(/\+?\d[\d\s\-()]{7,18}/g);
  const extractPhone = () => {
    if (!phoneMatch) return null;
    for (const cand of phoneMatch) {
...
async function executeAction(action, m, sock) {
  ...
  switch (action.type) {
    ...
    case "ADD": {
      ...
      if (num.length < 8) return [{ ok: false, msg: "Nomor tidak valid" }];
    ...
    case "STICKER": {
      ...
      await sock.sendMessage(
        m.chat,
        {
          ...
          packname: config.bot?.name || "Xerophis Bot",
          author: "AutoAI",
        },
        { quoted: m },
      );
