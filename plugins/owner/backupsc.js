...
async function handler(m, { sock }) {
  ...
  try {
    ...
    const timestamp = moment().tz((process.env.BOT_TZ || "Africa/Bamako")).format("YYYY-MM-DD_HH-mm-ss");
    const botName =
      config.bot?.name?.replace(/[^a-zA-Z0-9]/g, "") || "XerophisBot";
    ...
    await sock.sendMessage(
      m.chat,
      {
        ...
        caption:
          ...
          `┃ 📊 sɪᴢᴇ: \`${fileSizeMB} MB\`\n` +
          `┃ 📁 ꜰɪʟᴇ: \`${fileCount}\`\n` +
          `┃ 📅 ᴛᴀɴɢɢᴀʟ: \`${moment().tz((process.env.BOT_TZ || "Africa/Bamako")).format("DD/MM/YYYY")}\`\n` +
          `╰┈┈⬡`,
        contextInfo: {
