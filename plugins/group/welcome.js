...
function resolvePlaceholders(
  ...
  const now = moment().tz((process.env.BOT_TZ || "Africa/Bamako"));
  const dayNames = {
    Sunday: "Minggu",
    Monday: "Senin",
    Tuesday: "Selasa",
  ...
  return template
    ...
    .replace(/{bot}/gi, config.bot?.name || "Xerophis")
...
Tanoshii jikan o issho ni sugoso ne~`,
  ...
  msg += `> 📅 *Tanggal* : ${moment().tz((process.env.BOT_TZ || "Africa/Bamako")).format("DD/MM/YYYY")}\n`;
    ...
    if (welcomeType === 2) {
      ...
      await sock.sendMessage(groupJid, {
        interactiveMessage: {
          ...
          footer: { text: config.bot?.name || "Xerophis Bot" },
