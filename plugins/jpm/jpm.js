...
async function startAutoJpmSession(m, sock, db) {
  ...
  return sendInteractiveMessage(m, sock, {
    title: `🔄 ${config.bot?.name || "Xerophis Bot"} AutoJPM`,
    body,
    footer: `${config.bot?.name || "Xerophis Bot"} AutoJPM`,
    buttons,
  });
}
...
async function handleSetDelay(m, sock, db, input) {
  ...
  if (!input) {
    ...
    return sendInteractiveMessage(m, sock, {
      title: `⏱️ ${config.bot?.name || "Xerophis Bot"} Delay`,
      body,
      footer: `${config.bot?.name || "Xerophis Bot"} JPM System`,
      buttons,
    });
  }
