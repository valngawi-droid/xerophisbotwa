import { TZ_LABEL } from "../../src/lib/xerophis-region.js";
...
Kamu bisa pakai aku buat cari info, ambil data, atau bantu hal-hal sederhana langsung lewat WhatsApp — praktis tanpa ribet.`;
  const botInfoLines = [
    ...
    `👑 ᴏᴡɴᴇʀ    : ${botConfig.owner?.name || "Xerophis Bot"}`,
  ];
  const userInfoLines = [
    `🙋 ɴᴀᴍᴀ     : ${m.pushName}`,
  ...
  userInfoLines.push(`🕒 ᴡᴀᴋᴛᴜ    : ${timeStr} ${TZ_LABEL}`);
  ...
  if (useBracketBoxStyle) {
    ...
    txt += `\n\n╭─〔 🤖 *ʙᴏᴛ ɪɴꜰᴏ* 〕\n`;
    txt += `*│* 🖐 ɴᴀᴍᴀ     : *${botConfig.bot?.name || "Xerophis Bot"}*\n`;
    ...
    txt += `*│* 👑 ᴏᴡɴᴇʀ    : *${botConfig.owner?.name || "Xerophis Bot"}*\n`;
    txt += `╰────────────────⬣\n\n`;
    txt += `╭─〔 👤 *ᴜsᴇʀ ɪɴꜰᴏ* 〕\n`;
    txt += `*│* 🙋 ɴᴀᴍᴀ     : *${m.pushName}*\n`;
...
function getContextInfo(
  ...
  const saluranName =
    botConfig.saluran?.name || botConfig.bot?.name || "Xerophis Bot";
  ...
  const ctx = {
    ...
    isForwarded: true,
    externalAdReply: {
      title: botConfig.bot?.name || "Xerophis Bot",
                  ...
                  nativeFlowMessage: {
                    messageParamsJson: JSON.stringify({
                      ...
                      tap_target_configuration: {
                        ...
                        canonical_url: "https://whatsapp.com/channel/0029VbB37bgBfxoAmAlsgE0t",
